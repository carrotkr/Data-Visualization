function Castro_Upper_Market() {
	...
	// Fetch data.
	d3.csv('Castro_Upper_Market.csv').then(function(csv_data) {
    var dataset = d3.nest()
      .key(function(d) {
    ...
    // // Create a left axis generator using the yScale.
    // var leftAxis = d3.axisLeft(yScale);
    // d3.select('svg')
    //     .append('g').attr('id', 'left-axis')
    //     .call(leftAxis);

    // // Map skills to horizontal positions.
    // var skillScale = d3.scaleBand();
    // // Create an array of skill strings.
    // var skillDomain = dataset.map(function(skill) {
    //     return skill.key
    // });
    // // Set the range of the skillScale to 0->800.
    // skillScale.range([0, WIDTH]);
    // // Set the domain to be the array of skill strings.
    // skillScale.domain(skillDomain);
    
    // // Create a bottom axis generator that uses the skillScale.
    // var bottomAxis = d3.axisBottom(skillScale);
    // d3.select('svg')
    //     .append('g').attr('id', 'bottom-axis')
    //     .call(bottomAxis)
    //     // Move it to the bottom of the svg.
