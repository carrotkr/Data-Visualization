...
d3.csv("mrc_table_vis_1.csv", convert).then(draw);
...
function convert(row) {
  ...
  switch(row.name) {
    case "Columbia University In The City Of New York":
    case "Cornell University":
    case "Colgate University":
    case "New York University":
    case "State University Of New York At Stony Brook":
      keep.label = true;
      break;
    default:
      keep.label = false;
...
function draw(data) {
  ...
  data = data.filter(row => row.state === "NY");
